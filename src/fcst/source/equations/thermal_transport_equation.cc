...
//    - Id: $Id: thermal_transport_equation.cc 2605 2014-08-15 03:36:44Z secanell $
//
//---------------------------------------------------------------------------
...
template <int dim>
NAME::ThermalTransportEquation<dim>::ThermalTransportEquation(FuelCell::SystemManagement& system_management)
:
                                                                        ...
                                                                        FuelCellShop::Layer::BaseLayer<dim>* const layer)
{
    ...
    for (unsigned int q=0; q < this->n_q_points_bdry; ++q)
    {
        ...
        if (!bdry_residual_counter)
            this->normal_vectors[q] = (bdry_info.fe(t_rev.fetype_index)).normal_vector(q);
    }
}
