...
NAME::Platinum::exchange_current_density(const double& Temp) const
{
    if (name_reaction_kinetics == ORR)
    {
        ...
        {
            double A = 1.69278e-10;
            double E_a = 80987.61;
            return (A * exp( (-E_a/Constants::R()) * ( 1.0/Temp - 1.0/303.15 ) ));
        }
        else if (method_kinetics_ORR == "Parthasarathy_hcd")
        {
            double A = 2.83792e-7;
        ...
        else if (method_kinetics_ORR == "Neyerlin")
        {
            ...
            double E_rev = 67000.;
            double exponential = (-E_rev/(Constants::R()*Temp))*(1.0-(Temp/353.0) );
...
NAME::Platinum::derivative_exchange_current_density(const double& Temp) const
{
    if (name_reaction_kinetics == ORR)
    {
        ...
        else if (method_kinetics_ORR == "Parthasarathy")
        {
            double A = 1.69278e-10;
            double E_a = 80987.61;
            return (A * exp( (-E_a/Constants::R()) * ( 1.0/Temp - 1.0/303.15 ) ) * (E_a/(Constants::R() * Temp * Temp)));
        }
        else if (method_kinetics_ORR == "Parthasarathy_hcd")
        {
            double A = 2.83792e-7;
        ...
        else if (method_kinetics_ORR == "Neyerlin")
        {
            double i_ref_standard = 2.47e-8;
            double E_rev = 67000.;
            double exponential = (-E_rev/(Temp*Constants::R()))*(1.0-(Temp/353.0) );
            return (i_ref_standard*exp(exponential)*(E_rev/(Constants::R()*Temp)));
        }
    }
