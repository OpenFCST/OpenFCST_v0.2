                                                   ...
                                                   std::vector<double>&       dynamic_viscosity) const
{
       ...
       get_collision_integral(temperature,
                              omega);
                                                                 ...
                                                                 std::vector<double>&       dst) const
{
       ...
       get_Dcollision_integral_Dtemperature(temperature,
                                            Domega_DT);
                                     ...
                                     std::vector<double>&       dynamic_viscosity) const
{
       ...
       if     ( dynamic_viscosity_mode == "Sutherland"    )
              get_Sutherland_dynamic_viscosity(temperature,
                                               dynamic_viscosity);

       else if( dynamic_viscosity_mode == "ChapmanEnskog" )
              get_ChapmanEnskog_dynamic_viscosity(temperature,
                                                  dynamic_viscosity);
                                                   ...
                                                   std::vector<double>&       dst) const
{
       ...
       if     ( dynamic_viscosity_mode == "Sutherland"    )
              get_DSutherland_dynamic_viscosity_Dtemperature(temperature,
                                                             dst);

       else if( dynamic_viscosity_mode == "ChapmanEnskog" )
              get_DChapmanEnskog_dynamic_viscosity_Dtemperature(temperature,
                                                                dst);
                                                   ...
                                                   std::vector<double>&       thermal_conductivity) const
{
       ...
       get_specific_heat_capacity(temperature,
                                                                 ...
                                                                 std::vector<double>&       dst) const
{
       ...
       get_Dspecific_heat_capacity_Dtemperature(temperature,
                                                Dspecific_heat_capacity_DT);

       std::vector<double> Ddynamic_viscosity_DT;
       Ddynamic_viscosity_DT.resize(temperature.size());

       get_DSutherland_dynamic_viscosity_Dtemperature(temperature,
                                                      Ddynamic_viscosity_DT);
                                        ...
                                        std::vector<double>&       thermal_conductivity) const
{
       ...
       if     ( thermal_conductivity_mode == "Sutherland"    )
              get_Sutherland_thermal_conductivity(temperature,
                                                  thermal_conductivity);

       else if( thermal_conductivity_mode == "ChapmanEnskog" )
              get_ChapmanEnskog_thermal_conductivity(temperature,
                                                     thermal_conductivity);
                                                      ...
                                                      std::vector<double>&       dst) const
{
       ...
       if     ( thermal_conductivity_mode == "Sutherland"    )
              get_DSutherland_thermal_conductivity_Dtemperature(temperature,
                                                                dst);

       else if( thermal_conductivity_mode == "ChapmanEnskog" )
              get_DChapmanEnskog_thermal_conductivity_Dtemperature(temperature,
                                                                   dst);
                                                  ...
                                                  std::vector<double>&       dst) const
{
       ...
       get_D2specific_heat_capacity_Dtemperature2(temperature,
                                                  D2specific_heat_capacity_DT2);
                                                                 ...
                                                                 std::vector<double>&       dst) const
{
       ...
       get_water_vapor_saturation_pressure(temperature,
...
NAME::PureGas::get_D2specific_heat_capacity_Dtemperature2(const double& temperature) const
{
       return (   2.0*c_2
                  +
                  6.0*c_3 * (temperature/1000.0)   ) / (1000.0*1000.0);
}
                                                          ...
                                                          std::vector<double>&       dst) const
{
       ...
       for(unsigned int q = 0; q < temperature.size(); ++q)
              dst[q] = (   2.0*c_2
                           +
                           6.0*c_3 * (temperature[q]/1000.0)   ) / (1000.0*1000.0);
}