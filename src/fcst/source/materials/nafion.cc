...
NAME::Nafion::proton_conductivity(double& S) const
{
    ...
    if (method_conductivity == "Constant")
        ...
        S = (0.005139*lambda - 0.00326)* exp(1268.0*(1.0/303.0 - 1.0/T) ); //T in kelvin
...
NAME::Nafion::proton_conductivity(std::vector<double>& S_vec) const
{
    ...
    if (method_conductivity == "Constant")
        ...
        for (unsigned int i=0; i<S_vec.size(); ++i)
            S_vec[i] = (0.005139*(lambda_var[i]) - 0.00326)* exp(1268.0*(1.0/303.0 - 1.0/(T_var[i])) ); //T in kelvin
                    
    else if (method_conductivity == "NRE211")
        for (unsigned int i=0; i<S_vec.size(); ++i)
            S_vec[i] = ((-0.00010125)*pow((lambda_var[i]),2.0) + 0.01052*(lambda_var[i]) - 0.020634)* exp(751.5412*(1.0/303.0 - 1.0/(T_var[i])) ); //T in kelvin
                    
    else if (method_conductivity == "Iden11")
        for (unsigned int i=0; i<S_vec.size(); ++i)
        {
            double RH(0.0); // activity of water equivalent to Relative Humidity 
            if (lambda_var[i] < 13.0)
                RH = ( 0.000093561521965*pow(lambda_var[i],3.0) - 0.008649693377387*pow(lambda_var[i],2.0) + 0.183227305676982*lambda_var[i] - 0.125414456218017 ) * 100.0;
            else
                RH = 100.0;
    
            S_vec[i] = ( 1.93134146e-7*pow(RH,3.0) - 6.73473135e-6*pow(RH,2.0) + 0.000745491046859*RH - 0.007977537491969 ) * exp (751.5412* (1.0/353.0 - 1.0/T_var[i]) );
        }
    ...
    for (unsigned int i=0; i<derivative_flags.size(); ++i)
    {
        ...
        if (derivative_flags[i] == temperature_of_REV)
        {
            if (method_conductivity == "Constant")
                ...
                for (unsigned int j=0; j<T_var.size(); ++j)
                    derivS_vec[j] = (0.005139*lambda_var[j] - 0.00326)* exp(1268.0*(1.0/303.0 - 1.0/(T_var[j])) ) *1268.0 * (1.0/pow((T_var[j]),2.0));
        ...
        else if (derivative_flags[i] == membrane_water_content)
        {
            if (method_conductivity == "Constant")
                ...
                for (unsigned int j=0; j<lambda_var.size(); ++j)
                    derivS_vec[j] = 0.005139* exp(1268.0*(1.0/303.0 - 1.0/(T_var[j])) );
...
NAME::Nafion::water_diffusivity(double& D_w) const
{
    ...
    if (method_diffusivity == "Constant")
        ...
        D_w = 1.e-6*(2.563 - 0.33*lambda + 0.0264*pow(lambda,2.0) - 0.000671*pow(lambda,3.0))* exp(2416.0*(1.0/303.0 - 1.0/T));

    else if (method_diffusivity == "Motupally")
        if (lambda <= 3. && lambda > 0.)
            D_w = 3.10e-3*lambda*(-1.0 + exp(0.28*lambda))*exp(-2436.0/T);
        else
            D_w = 4.17e-4*lambda*(1.0 + 161.0*exp(-lambda))*exp(-2436.0/T);
...
NAME::Nafion::water_diffusivity(std::vector<double>& Dw_vec) const
{
    ...
    if (method_diffusivity == "Constant")
        for (unsigned int i=0; i<Dw_vec.size(); ++i)
            Dw_vec[i] = diffusion_w;

    else if (method_diffusivity == "Springer")
        for (unsigned int i=0; i<Dw_vec.size(); ++i)
            Dw_vec[i] = 1.e-6*(2.563 - 0.33*lambda_var[i] + 0.0264*pow(lambda_var[i],2.0) - 0.000671*pow(lambda_var[i],3.0))* exp(2416.0*(1.0/303.0 - 1.0/T_var[i]));
        
    else if (method_diffusivity == "Motupally")
        for (unsigned int i=0; i<Dw_vec.size(); ++i)
        {
            if (lambda_var[i] <= 3. && lambda_var[i] > 0.)
                Dw_vec[i] = 3.10e-3*lambda_var[i]*(-1.0 + exp(0.28*lambda_var[i]))*exp(-2436.0/T_var[i]);
            else
                Dw_vec[i] = 4.17e-4*lambda_var[i]*(1.0 + 161.0*exp(-lambda_var[i]))*exp(-2436.0/T_var[i]);
        }
...
NAME::Nafion::oxygen_diffusivity(double& D_oxy) const
{
    ...
    D_oxy = D_O2*exp(-(39760.0/Constants::R())*(1.0/T - 1.0/353.0));        // D_02 = 9.726e-6 [cm^2/s] (Default value)
}
...
NAME::Nafion::oxygen_diffusivity(std::vector<double>& Doxy_vec) const
{
    ...
    for (unsigned int i=0; i<Doxy_vec.size(); ++i)
        Doxy_vec[i] = D_O2*exp(-(39760.0/Constants::R())*(1.0/T_var[i] - 1.0/353.0));         // D_02 = 9.726e-6 [cm^2/s] (Default value)
}
