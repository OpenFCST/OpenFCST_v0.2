...
NAME::DualPathKinetics::derivative_current (std::map< VariableNames, std::vector<double> >& dcoef_du)
{
    ...
    Assert( std::find(derivative_flags.begin(), derivative_flags.end(), hydrogen_concentration) == derivative_flags.end(),
            ExcMessage("Derivative w.r.t. hydrogen concentration is not currently implemented, instead it returns the derivative w.r.t. hydrogen molar fraction.") );
    Assert( electrolyte->get_H_H2() != 0.0, ExcMessage("Derivatives at the moment are defined only for Thin fim (Henry's law) case."));
