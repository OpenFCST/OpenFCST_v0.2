...
template <int dim>
void
NAME::DummyCL<dim>::derivative_effective_proton_conductivity(std::map< VariableNames, std::vector<double> >& prop_eff) const
{
    Assert( false, ExcNotImplemented());
    // Issue of internal vector size. Normally not required to be initialized inside the conventional CL but how to tackle it here..Madhur
    //Assert( this->derivative_flags.size()!=0, ExcMessage("set_derivative_flags has not been probably called in DummyCL::derivative_effective_proton_conductivity."));
}
