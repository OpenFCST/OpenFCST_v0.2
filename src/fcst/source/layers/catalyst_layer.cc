...
#include "../../include/application_core/fe_vectors.h"
...
NAME::CatalystLayer<dim>::set_solution(const std::vector< SolutionVariable >& sols)
{
    ...
    for (unsigned int i=0; i < sols.size(); ++i)
    {
        ...
        else if( sols[i].get_variablename() == hydrogen_concentration )
        {
               ...
               {
                      hydrogen_concentration_old_GL[q] = Constants::R()*this->constant_solutions.at(temperature_of_REV)*sols[i][q]/(this->electrolyte->get_H_H2()*1.0e-6);
               }
        ...
        else if( sols[i].get_variablename() == oxygen_concentration )
        {
               ...
               {
                      oxygen_concentration_old_GL[q] = Constants::R()*this->constant_solutions.at(temperature_of_REV)*sols[i][q]/(this->electrolyte->get_H_O2()*1.0e-6);
               }
               ...
               reactant_concentrations.push_back( FuelCellShop::SolutionVariable( oxygen_concentration_old_GL,
                                                                                  VariableNames::oxygen_concentration) );
        }
        ...
        else if (sols[i].get_variablename() == oxygen_molar_fraction)
        {
            ...
            Assert(((this->reactant == nothing) || (this->reactant == oxygen_molar_fraction)), ExcMessage("Cannot supply multiple reactants to MultiScaleCL") );
        ...
        else if (sols[i].get_variablename() == hydrogen_molar_fraction)
        {
            ...
            Assert( ((this->reactant == nothing) || (this->reactant == hydrogen_molar_fraction)), ExcMessage("Cannot supply multiple reactants to MultiScaleCL") );
    ...
    Assert( reactant_concentrations.size() > 0, ExcMessage("Atleast one reactant concentration/molar fraction is required in input vector to CatalystLayer::set_solution.") );
