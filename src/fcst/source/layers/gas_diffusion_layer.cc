...
template <int dim>
void 
NAME::GasDiffusionLayer<dim>::effective_gas_diffusivity(Table<2, double>& D_eff ) const
{
    this->gas_diffusion_coefficients(D_eff);
}

//---------------------------------------------------------------------------
template <int dim>
void 
NAME::GasDiffusionLayer<dim>::effective_gas_diffusivity(Table< 2, Tensor<2,dim> > &D_eff) const
{
    ...
    Table<2, double> D;
    this->gas_diffusion_coefficients(D);
    Tensor<2,dim> tortuosity;
    tortuosity = Constants::unit_tensor();
    ...
    for (unsigned int i=0; i<this->gases.size(); i++)
    {
        for (unsigned int j=0; j<this->gases.size(); j++)
        {
            if( i != j )
            {
                D_eff(i,j) = ( 1.0 / (porosity*D(i,j)) ) * tortuosity;
            }
        }
    }
