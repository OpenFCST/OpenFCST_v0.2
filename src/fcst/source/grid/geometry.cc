...
NAME::GridBase<dim>::declare_GridGenerator_parameters(ParameterHandler& param)
{
    param.enter_subsection("Grid generation");
    {
        param.declare_entry("Type of mesh",
                            "Cathode",
                            Patterns::Selection("GridExternal | Cathode | Anode | CathodeMPL | Pemfc | PemfcMPL | Agglomerate | GridTest | HyperCube"),
...
NAME::GridBase<dim>::get_material_id (const std::string material) const
{
    ...
    if (material.compare("Cathode GDL") == 0)
    {
    ...
    {
        FcstUtilities::log<<"The material_id requested is not defined"<<std::endl;
        abort();
    }
