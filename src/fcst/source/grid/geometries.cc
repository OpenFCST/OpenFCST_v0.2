...
template <int dim>
NAME::GridExternal<dim> const* NAME::GridExternal<dim>::PROTOTYPE = new NAME::GridExternal<dim>(true);

//---------------------------------------------------------------------------
template <int dim>
void
NAME::GridExternal<dim>::generate_grid( Triangulation<dim>& triangulation) const
{
...
template <int dim>
NAME::HyperCube<dim> const* NAME::HyperCube<dim>::PROTOTYPE = new NAME::HyperCube<dim>(true);

//---------------------------------------------------------------------------
template <int dim>
void
NAME::HyperCube<dim>::generate_grid( Triangulation<dim>& triangulation) const
{
...
//---------------------------------------------------------------------------
template <int dim>
const std::string NAME::Cathode<dim>::concrete_name ("Cathode");

template <int dim>
NAME::Cathode<dim> const* NAME::Cathode<dim>::PROTOTYPE = new NAME::Cathode<dim>(true);

//---------------------------------------------------------------------------
template <int dim>
void
NAME::Cathode<dim>::generate_grid(Triangulation<dim> &triangulation) const
{
    ...
    #ifdef _2D_
    // First need to create a vector containing number of cells in x and y directions
    ...
    GridGenerator::subdivided_hyper_rectangle(triangulation, mesh, Point<2> (0,0), Point<2> (this->l_gdl_c + std::accumulate(this->l_cat_c.begin(),this->l_cat_c.end(),0.0), this->l_land_c + this->l_channel_c), false);
    
    // Loop over the cells checking to see if the cell vertices are close to the FC boundaries
    ...
    for (; cell!=endc; ++cell)
    {
        ...
        else if(vert(0) < std::accumulate(this->l_cat_c.begin(),this->l_cat_c.end(), 0.0))
        {
            for(unsigned int j = 0; j < this->l_cat_c.size(); j++)
            {
        ...
        for (unsigned int j=0; j < GeometryInfo<dim>::lines_per_cell; j++)
        {
            ...
            for(unsigned int j = 0; j < this->l_cat_c.size(); j++)
            {
                if (vert(0) == std::accumulate(this->l_cat_c.begin(),this->l_cat_c.begin() + j,0.0))
                {
                    ...
                    if (vert(0) == std::accumulate(this->l_cat_c.begin(),this->l_cat_c.begin() + j,0.0))
                    {
                        line->set_boundary_indicator(this->c_GDL_CL_bid);                                                         // CCL|CCL or c_GDL_CL_bid
                        continue;
                    }
                    vert = line->vertex(0);
                }
            }
            if (std::abs(vert(0) - std::accumulate(this->l_cat_c.begin(),this->l_cat_c.end(),0.0) - this->l_gdl_c) < 1e-6)
            {
                vert = line->vertex(1);
                if (std::abs(vert(0)  - std::accumulate(this->l_cat_c.begin(),this->l_cat_c.end(),0.0) - this->l_gdl_c) < 1e-6)
                {
...
template <int dim>
NAME::CathodeMPL<dim> const* NAME::CathodeMPL<dim>::PROTOTYPE = new NAME::CathodeMPL<dim>(true);

//---------------------------------------------------------------------------
template <int dim>
void
NAME::CathodeMPL<dim>::generate_grid(Triangulation<dim> &triangulation) const
{
...
//---------------------------------------------------------------------------
template <int dim>
const std::string NAME::Anode<dim>::concrete_name ("Anode");

template <int dim>
NAME::Anode<dim> const* NAME::Anode<dim>::PROTOTYPE = new NAME::Anode<dim>(true);

//---------------------------------------------------------------------------
template <int dim>
void
NAME::Anode<dim>::generate_grid(Triangulation<dim> &triangulation) const
{
    ...
    for(unsigned int i = 0; i < this->num_vert; i++)
    {
        part.push_back((this->l_land_a + this->l_channel_a)/this->num_vert);
    }
    mesh.push_back(part);
    
    // Now create the mesh using the lengths to make the upper corner point
    GridGenerator::subdivided_hyper_rectangle(triangulation, mesh, Point<2> (0,0), Point<2> (this->l_gdl_a + this->l_cat_a, this->l_land_a + this->l_channel_a), false);
    ...
    Point<dim> vert;
    //calculate current cell height
    const double initial_cell_height = (this->l_land_a + this->l_channel_a)/this->num_vert;
    ...
    for (; cell!=endc; ++cell)
    {
        ...
        {
            vert = cell->vertex(i);
            //check to see if current vertex is close to anode current collector/gas channel boundary
            if (std::abs(vert(1) - this->l_land_a - 0.00001) <= initial_cell_height/2)
            {
        ...
        //loop has checked all vertices and is now on last (top right corner)
        //use this to set material_id
        if (vert(0) > this->l_cat_a)
        ...
        for (unsigned int j=0; j < GeometryInfo<dim>::lines_per_cell; j++)
        {
            ...
            if (vert(0) == this->l_cat_a)
            {
                ...
                if (vert(0) == this->l_cat_a)
                {
                    line->set_boundary_indicator(this->a_CL_GDL_bid);
                    continue;
                }
                vert = line->vertex(0);
            }
            if (std::abs(vert(0) - this->l_cat_a - this->l_gdl_a) < 0.00001)
            {
                if (std::abs(vert(0) - this->l_cat_a - this->l_gdl_a) < 0.00001)
                {
                    if (vert(1) < this->l_land_a)
...
//---------------------------------------------------------------------------
template <int dim>
const std::string NAME::AnodeMPL<dim>::concrete_name ("AnodeMPL");

template <int dim>
NAME::AnodeMPL<dim> const* NAME::AnodeMPL<dim>::PROTOTYPE = new NAME::AnodeMPL<dim>(true);


//---------------------------------------------------------------------------
template <int dim>
void
NAME::AnodeMPL<dim>::generate_grid(Triangulation<dim> &triangulation) const
{
...
//---------------------------------------------------------------------------
template <int dim>
const std::string NAME::Pemfc<dim>::concrete_name ("Pemfc");

template <int dim>
NAME::Pemfc<dim> const* NAME::Pemfc<dim>::PROTOTYPE = new NAME::Pemfc<dim>(true);

//---------------------------------------------------------------------------
template <int dim>
void
NAME::Pemfc<dim>::generate_grid(Triangulation<dim> &triangulation) const
{
    ...
    GridGenerator::subdivided_hyper_rectangle(triangulation, mesh, Point<2> (0,0),
                                              Point<2> (this->l_gdl_a + this->l_cat_a + this->l_mem + std::accumulate(this->l_cat_c.begin(),this->l_cat_c.end(),0.0) + this->l_gdl_c, this->l_land_c + this->l_channel_c), false);
    ...
    for (; cell!=endc; ++cell)
    {
        ...
        if (vert(0) >= 0 &&
            ...
            cell->set_material_id(this->a_GDL_mid);
                 ...
                 vert(0) <= (this->l_gdl_a + this->l_cat_a))
            cell->set_material_id(this->a_CL_mid);
        else if (vert(0) > (this->l_gdl_a + this->l_cat_a) &&
                 vert(0) <= (this->l_gdl_a + this->l_cat_a + this->l_mem ))
            cell->set_material_id(this->membrane_mid);
        else if(vert(0) > this->l_gdl_a + this->l_cat_a + this->l_mem  &&
                vert(0) <= this->l_gdl_a + this->l_cat_a + this->l_mem + std::accumulate(this->l_cat_c.begin(),this->l_cat_c.end(),0.0))
        {
            for(unsigned int j = 0; j < this->l_cat_c.size(); j++)
            {
                if (vert(0) > this->l_gdl_a + this->l_cat_a + this->l_mem + std::accumulate(this->l_cat_c.begin(),this->l_cat_c.begin() + j, 0.0) && 
                    vert(0) <= this->l_gdl_a + this->l_cat_a + this->l_mem + std::accumulate(this->l_cat_c.begin(),this->l_cat_c.begin() + j + 1, 0.0))
                    cell->set_material_id(this->c_CL_mid.at(j));
            }
        }
        else
        ...
        for (unsigned int j=0; j < GeometryInfo<dim>::lines_per_cell; j++)
        {
            ...
            if (std::abs(vert(0) - this->l_gdl_a - this->l_cat_a) < 0.00001)
            {
                vert = line->vertex(1);
                if (std::abs(vert(0) - this->l_gdl_a - this->l_cat_a) < 0.00001)
                {
            ...
            if (std::abs(vert(0) - this->l_gdl_a - this->l_cat_a - this->l_mem) < 0.00001)
            {
                vert = line->vertex(1);
                if (std::abs(vert(0) - this->l_gdl_a - this->l_cat_a - this->l_mem) < 0.00001)
                {
            ...
            for(unsigned int j = 0; j < this->l_cat_c.size(); j++)
                {
                if (std::abs(vert(0) - this->l_gdl_a - this->l_cat_a - this->l_mem - std::accumulate(this->l_cat_c.begin(),this->l_cat_c.begin() + j,0.0)) < 0.00001)
                {
                    vert = line->vertex(1);
                    if (std::abs(vert(0) - this->l_gdl_a - this->l_cat_a - this->l_mem - std::accumulate(this->l_cat_c.begin(),this->l_cat_c.begin() + j,0.0)) < 0.00001)
                    {
            ...
            if (std::abs(vert(0) - this->l_gdl_c - std::accumulate(this->l_cat_c.begin(),this->l_cat_c.end(),0.0) - this->l_mem - this->l_cat_a - this->l_gdl_a) < 0.00001)
            {
                vert = line->vertex(1);
                if (std::abs(vert(0) - this->l_gdl_c - std::accumulate(this->l_cat_c.begin(),this->l_cat_c.end(),0.0) - this->l_mem - this->l_cat_a - this->l_gdl_a) < 0.00001)
                {
...
template <int dim>
NAME::PemfcMPL<dim> const* NAME::PemfcMPL<dim>::PROTOTYPE = new NAME::PemfcMPL<dim>(true);

//---------------------------------------------------------------------------
template <int dim>
void
NAME::PemfcMPL<dim>::generate_grid(Triangulation<dim> &triangulation) const
{
...
template <int dim>
NAME::Agglomerate<dim> const* NAME::Agglomerate<dim>::PROTOTYPE = new NAME::Agglomerate<dim>(true);

//---------------------------------------------------------------------------
template <int dim>
void
NAME::Agglomerate<dim>::generate_grid(Triangulation<dim> &triangulation) const
{
...
template <int dim>
NAME::GridTest<dim> const* NAME::GridTest<dim>::PROTOTYPE = new NAME::GridTest<dim>(true);

//---------------------------------------------------------------------------
template <int dim>
void
NAME::GridTest<dim>::generate_grid( Triangulation<dim>& triangulation) const
{
...
template class NAME::Cathode<deal_II_dimension>;
template class NAME::CathodeMPL<deal_II_dimension>;
template class NAME::Anode<deal_II_dimension>;
template class NAME::AnodeMPL<deal_II_dimension>;
template class NAME::Pemfc<deal_II_dimension>;
