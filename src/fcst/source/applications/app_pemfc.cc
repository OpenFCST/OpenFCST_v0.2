...
//    - $Id: app_pemfc.cc 2605 2014-08-15 03:36:44Z secanell $
//
//---------------------------------------------------------------------------
...
NAME::AppPemfc<dim>::AppPemfc(boost::shared_ptr<FuelCell::ApplicationCore::ApplicationData> data)
  ...
  proton_transport(this->system_management),
  lambda_transport(this->system_management),
  electron_transport(this->system_management),
  reaction_source_terms(this->system_management),
  sorption_source_terms(this->system_management),
  ficks_oxygen_nitrogen(this->system_management, &oxygen, &nitrogen),
  ficks_water_nitrogen(this->system_management, &water, &nitrogen),
  ficks_water_hydrogen(this->system_management, &water, &hydrogen),
...
template <int dim>
void
NAME::AppPemfc<dim>::_initialize(ParameterHandler& param)
{
                              ...
                              const FuelCell::ApplicationCore::FEVectors &vectors)
{
    ...
    // --- Find solution ---
    FuelCell::ApplicationCore::FEVector solution = vectors.vector( vectors.find_vector("Solution") );
