...
//---------------------------------------------------------------------------
double
NAME::AgglomerateBase::compute_n() const
{
    double n_agg = 0.0;
...
//---------------------------------------------------------------------------
double
NAME::AgglomerateBase::compute_dn_depsilon_agg() const
{
    double dn_de = 0.0;



    dn_de = this->layer->get_properties()[CLPropNames::solid_fraction]
...
//---------------------------------------------------------------------------
double
NAME::AgglomerateBase::compute_thickness_agg()
{
    ...
    throw std::runtime_error("Agglomerate film thickness has not converged (r_agg = " +std::to_string(get_radius()) + "[nm])" );
}

//---------------------------------------------------------------------------
double
NAME::AgglomerateBase::compute_epsilon_agg()
{
    //
    ...
    {
        n_agg = compute_n();
        epsilon_i = epsilon_0 - (compute_epsilon_N(delta_agg, epsilon_0) - props[CLPropNames::ionomer_fraction]) / compute_depsilonN_depsilon_agg();
...
//---------------------------------------------------------------------------
double
NAME::AgglomerateBase::compute_epsilon_N(const double delta_agg,
...
//---------------------------------------------------------------------------
double
NAME::AgglomerateBase::compute_depsilonN_dthickness(const double thickness_agg) const
{
    double value = 0.0;
...
//---------------------------------------------------------------------------
double
NAME::AgglomerateBase::compute_depsilonN_depsilon_agg() const
{
    double value = 0.0;
        ...
        * (pow(r_agg * 1e-7, 3.0))
        +
        compute_dn_depsilon_agg() * (4.0 / 3.0) * pi
