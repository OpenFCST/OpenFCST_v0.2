...
/**
 * Helper functions computing the
 * desired data in each quadrature
 * point of a mesh entity by calling
 * @p FEValuesBase::get_function_values(),
 * @p FEValuesBase::get_function_grads(),
 * and
 * @p FEValuesBase::get_function_hessians().
 *
 * @param fe_values: The @p FEValues
 * object.
 *
 * @param fe_vector: The global finite
 * element function in the form of a global
 * nodal @p FEVector.
 *
 * @param local_dof_indices: The local DoF
 * indices associated with the current mesh entity.
 *
 * @param first_index: The first
 * index in @p local_dof_indices to be used.
 *
 * @param n_indices: The number of
 * indices in @p local_dof_indices to be
 * used.
 *
 * @param result: The result.
 * The access to @p ith component in
 * @p qth quadrature point is @p result[i][q].
 *
 * @author Guido Kanschat
 */
...
/**
 * A small structure collecting
 * the different @p BlockIndices
...
struct BlockInfo : public Subscriptor
{
  ...
  /**
   * A vector containing the internal
   * renumbering of degrees of freedom from the
   * standard ordering on a cell to a
   * block wise ordering on the same cell.
   *
  ...
  /**
   * Fill the structure globally with values describing the @p MGDoFHandler.
   */
  template<int dim, int spacedim>
 ...
 *
 * - @p MODE2: standard dealii local renumbering mode is activated if @p BlockInfo::local_renumbering.size() = 0
...
//@{
   ...
   *
   * This number is
   * @p deal_II_numbers::invalid_unsigned_int
...
//@{
  ...
  /**
   * Build internal structures @p fevalv
   * and allocate memory for
   * @p values,
   * @p gradients,
   * @p hessians.
   *
   * @param FE_VALUES is an object
   * of the actual type @p FEVALUES
   * which is
   * either
   * @p FEValues (cell)
   * or
   * @p FEFaceValues (face)
   * or
   * @p FESubfaceValues (subface).
   * @note @p FE_VALUES is really not
   * needed here and only serves to
   * define the actual type @p FEVALUES.
   *
   * @param fe is the finite
   * element (for instance, @p FESystem object)
   * stored in the @p DoFHandler.
   * It is used in the constructor
   * of @p FE_VALUES.
   *
   * @param mapping is the @p Mapping
...
//@{
  ...
  /**
   * Reinitialize internal data
   * for use on a subface of a face of a cell.
   */
  template<typename DHCellIterator, typename DHFaceIterator>
  void reinit(const DHCellIterator& c,
              const DHFaceIterator& f,
...
//@{
  ...
  /**
   * The smart pointer
   * to the @p FEVectors object
   * called @p global_data.
   */
