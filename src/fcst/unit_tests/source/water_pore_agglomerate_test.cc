...
void WaterPoreAgglomerateTest::testSolvePotentials(){
    ...
    //Test solution
    TEST_ASSERT_MSG(not isnan(agg.PorePotential[agg.New][agg.M-1][0]), "Values of potential are not numbers!");
...
void WaterPoreAgglomerateTest::testSolveO2(){
    ...
    //Test solution
    TEST_ASSERT_MSG(not isnan(agg.cO2[agg.New][agg.M-1][0]), "Values of potential are not numbers!");
